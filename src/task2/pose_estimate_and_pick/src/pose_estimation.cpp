...
void pose_estimation::update_points(const sensor_msgs::PointCloud2 cloud){
    CAMERA_FRAME.frame_id = "camera_color_optical_frame";
	  pcl::fromROSMsg (cloud, *scene_cloud);
    //std::cout << "height " << scene_cloud->height << "width " << scene_cloud->width;
  	return;
}
bool pose_estimation::serviceCb(task2::pose_estimation::Request &req, task2::pose_estimation::Response &res){
  ros::ServiceClient client = nh.serviceClient<task2::task1out>("prediction");
  task2::task1out srv;

  ros::ServiceClient client_grasp = nh.serviceClient<task2::data>("grasp");
  task2::data grasp_pose;
  ...
  if(client.call(srv)){ 
    ...
    float angle_dou = srv.response.angle_dou;
    ...
    for(int i=0;i<3;i++){
      ...
      for(j=0;j < count;j++){
        ...
        std::cout << "\nTF1: \n" << tf1;
        ini_guess_tf_cloud->header.frame_id = "camera_color_optical_frame";
        ...
        cout << "Tran" << endl;
        cout << final_tf(0,3) << " " << final_tf(1,3) << " " << final_tf(2,3) << endl;
        cout << "Rot" << endl;
        ...
        tf::Transform object_grasp = transform * david_tf;
        geometry_msgs::Pose pose;
        quaternionTFToMsg(object_grasp.getRotation(), pose.orientation); 
        tf::Vector3 pose_trans =  object_grasp.getOrigin();
	...
	float angle = 0;
	angle = angle_dou/180*3.14159;
	
	Eigen::Quaterniond gripper_quat = euler2Quaternion( -angle, (3.14159 / 2),0);
/*
    	pose.request.target_pose.position.x = pick_point_robot.point.x;
  	pose.request.target_pose.position.y = pick_point_robot.point.y;
    	pose.request.target_pose.position.z = pick_point_robot.point.z + 0.10;
    	pose.request.target_pose.orientation.x = quat.x();      //quat.x()
    	pose.request.target_pose.orientation.y = quat.y();      //quat.y()
    	pose.request.target_pose.orientation.z = quat.z();      //quat.z()
    	pose.request.target_pose.orientation.w = quat.w();      //quat.w()
*/

	grasp_pose.request.pose.position.x = pose.position.x;
	grasp_pose.request.pose.position.y = pose.position.y;
	grasp_pose.request.pose.position.z = pose.position.z;
	grasp_pose.request.pose.orientation.x = gripper_quat.x();
	grasp_pose.request.pose.orientation.y = gripper_quat.y();
	grasp_pose.request.pose.orientation.z = gripper_quat.z();
	grasp_pose.request.pose.orientation.w = gripper_quat.w();
	grasp_pose.request.id = 0;

	std::cout << "\nori_x: \n" << grasp_pose.request.pose.orientation.x;
	std::cout << "\nori_y: \n" << grasp_pose.request.pose.orientation.y;
	std::cout << "\nori_z: \n" << grasp_pose.request.pose.orientation.z;
	std::cout << "\nori_w: \n" << grasp_pose.request.pose.orientation.w;


	if(client_grasp.call(grasp_pose)){
		std::cout << "\nobj_id: \n" << grasp_pose.response.obj_id;
	}
        ...
        ROS_INFO("Success");
        if(i==0){
          cloud_source_trans_normals->header.frame_id = "camera_color_optical_frame";
...
void pose_estimation::load_models(){
    //////////////////Define model path/////////////
    string object_model_path("/home/sis/sis_competition_2020/catkin_ws/src/task2/FCN/models/");
